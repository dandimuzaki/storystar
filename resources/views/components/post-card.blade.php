<div class="flex h-fit bg-white border border-gray-200 rounded-lg shadow-sm">
    <div class="p-6 flex-1 grid gap-4">
        ...
        <div class="flex gap-4 text-gray-400 items-center">
            @if (auth()->user() && auth()->user()->id === $post->user->id && Carbon\Carbon::parse($post->published_at) > now())
                <div class="py-1 px-2 rounded bg-orange-500 text-white1`">Pending</div>
            @elseif (auth()->user() && auth()->user()->id === $post->user->id && Carbon\Carbon::parse($post->published_at) <= now())
